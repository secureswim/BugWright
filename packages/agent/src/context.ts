...
export const coderContext = (
  ...
  codingConstraints: { minimalPatch: true, noExecution: true, noPublishing: true },
});
export const testerContext = (issue: unknown, patch: PatchProposal, diff: string) => ({
  issue,
  codeChangeSummary: patch,
  currentDiff: diff.slice(0, 50000),
});
export const reviewerContext = (
  ...
  diff: string,
  tests: TestReport,
) => ({ originalIssue: issue, researchReports, currentDiff: diff, testReport: tests });
