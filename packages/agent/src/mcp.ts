...
export class McpTools {
  ...
  async connect(names: ServerName[]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        throw new Error(`MCP ${name} connection failed: ${detail}`, { cause: error });
      }
  ...
  async callTrusted(server: ServerName, name: string, args: Record<string, unknown> = {}) {
    return this.execute(server, name, args, undefined, 0);
  }
